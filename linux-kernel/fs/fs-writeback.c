...
EXPORT_SYMBOL(dirtytime_expire_interval);
...
// static bool lf_inode_io_list_move_locked(struct inode *inode,
// 				      struct bdi_writeback *wb,
// 				      struct slist_head *head)
// {
// 	// assert_spin_locked(&wb->list_lock);
// 	// assert_spin_locked(&inode->i_lock);
// 	// pr_info("inode_io_list_move_locked: inode %lu\n", inode->i_ino);

// 	// list_del_init(&inode->i_io_list);
// 	inode->i_io_list.prev = NULL;
// 	inode->i_io_list.next = NULL;
// 	slist_add(&inode->i_io_list, head);
...
EXPORT_SYMBOL(wb_start_background_writeback); //kiet

void (*inode_io_list_del_in_lustre)(struct inode *inode) = NULL;
EXPORT_SYMBOL(inode_io_list_del_in_lustre);
...
void inode_io_list_del(struct inode *inode)
{
	...
	if (inode_io_list_del_in_lustre)
		(*inode_io_list_del_in_lustre)(inode);
	...
	inode->i_state &= ~I_SYNC_QUEUED;
	// list_del_init(&inode->i_io_list);
	if (inode->i_io_list.next != NULL){
		if (inode->i_io_list.prev != NULL) 
			list_del_init_nocheck(&inode->i_io_list);
		else
			{
				inode->i_io_list.next = NULL;
			}
...
static void redirty_tail_locked(struct inode *inode, struct bdi_writeback *wb)
{
	...
	// lfinode_io_list_move_locked(inode, wb, &wb->b_dirty);
	inode->i_state &= ~I_SYNC_QUEUED;
}
			       ...
			       unsigned long dirtied_before)
{
	...
	while (!list_empty(delaying_queue)) {
		...
		if (inode_dirtied_after(inode, dirtied_before))
			break;
		spin_lock(&inode->i_lock);
		list_move(&inode->i_io_list, &tmp); //kiet
		moved++;
		inode->i_state |= I_SYNC_QUEUED;
		spin_unlock(&inode->i_lock);
...
EXPORT_SYMBOL(inode_wait_for_writeback); //Kiet
...
//	printk("[%s]: start\n", __func__);
	...
	while ((work = get_next_work_item(wb)) != NULL) {
		...
		/* taehwan code */
//		printk("[%s]: get_next_work_item(work_list)\n", __func__);
		/* taehwan code */
	...
	wrote += wb_check_background_flush(wb); //o ll_writepages
...
void wb_workfn(struct work_struct *work)
{
	...
	if (likely(!current_is_workqueue_rescuer() ||
		 ...
		 */
		do {
			pages_written = wb_do_writeback(wb); //ll_writepages
...
void __mark_inode_dirty(struct inode *inode, int flags)
{
	...
	if ((inode->i_state & flags) != flags) {
		...
		if (!was_dirty) {
			...
			// if (inode->i_state & I_DIRTY){
			// 	wakeup_bdi = lf_inode_io_list_move_locked(inode, wb,
			// 				       &wb->lustre_dirty);
			// }
			// else{
			// 	dirty_list = &wb->b_dirty_time;
