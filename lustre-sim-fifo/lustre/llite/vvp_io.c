                            ...
                            const struct cl_io_slice *ios)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_read_lock);
	RETURN(result);
}
...
static int vvp_io_write_lock_internal(const struct lu_env *env,
			     const struct cl_io_slice *ios)
{
	struct cl_io *io = ios->cis_io;
	loff_t start;
			     ...
			     const struct cl_io_slice *ios)
{
	...
	int ret;
	ktget(&localclock[0]);
	ret = vvp_io_write_lock_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_write_lock);
	return ret;
}
...
static int vvp_io_setattr_start_internal(const struct lu_env *env,
				const struct cl_io_slice *ios)
{
	struct cl_io *io = ios->cis_io;
	struct inode *inode = vvp_object_inode(io->ci_obj);
                              ...
                              const struct cl_io_slice *ios)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = vvp_io_setattr_start_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_setattr_start);
	return ret;
}
...
KTDEF(generic_file_read_iter);

static int vvp_io_read_start_internal(const struct lu_env *env,
			     const struct cl_io_slice *ios)
{
	struct vvp_io *vio = cl2vvp_io(env, ios);
	struct cl_io *io = ios->cis_io;
	...
	do {
		...
		ktget(&localclock[1]);
		ktput(localclock, generic_file_read_iter);
		if (result >= 0) {
			io->ci_nob += result;
                              ...
                              const struct cl_io_slice *ios)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = vvp_io_read_start_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_read_start);
	return ret;
}
...
static void vvp_set_pagevec_dirty(struct pagevec *pvec)
{
	...
	if (!vvp_account_page_dirtied) {
		for (i = 0; i < count; i++)
			__lustre_set_page_dirty_nobuffers(pvec->pages[i]);
		EXIT;
	}
#endif
	...
	if (mapping->host && dirtied) {
		...
		// __mark_inode_dirty
		__lustre_mark_inode_dirty(mapping->host, I_DIRTY_PAGES);
	}
#endif
	EXIT;
}
...
KTDEF(ll_merge_attr);
EXPORT_SYMBOL(ll_merge_attr_clock);

KTDEF(cl_io_commit_async);
EXPORT_SYMBOL(cl_io_commit_async_clock);
...
int vvp_io_write_commit(const struct lu_env *env, struct cl_io *io)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, cl_io_commit_async);
	...
	if (rc == -EDQUOT && !cl_io_is_mkwrite(io)) {
		...
		rc = vvp_io_commit_sync(env, io, queue,
					...
					vio->u.readwrite.vui_to);
		ktget(&localclock[1]);
		ktput(localclock, vvp_io_commit_sync);
		if (rc > 0) {
...
KTDEF(__generic_file_write_iter);
KTDEF(generic_write_sync);
KTDEF(vvp_io_write_commit);
KTDEF(directIO);

ssize_t
lustre_generic_file_direct_write(struct kiocb *iocb, struct iov_iter *from)
{
...
KTDEF(cl_io_lru_reserve);
EXPORT_SYMBOL(cl_io_lru_reserve_clock);

static int vvp_io_write_start_internal(const struct lu_env *env,
                              const struct cl_io_slice *ios)
{
	struct vvp_io		*vio   = cl2vvp_io(env, ios);
	struct cl_io		*io    = ios->cis_io;
	...
	// 	result = cl_io_lru_reserve(env, io, pos, cnt);
	// 	ktget(&localclock[1]);
	// 	ktput(localclock, cl_io_lru_reserve);
	// 	if (result)
	// 		RETURN(result);
	// }
	...
	if (vio->vui_iter == NULL) {
		...
		if (unlikely(lock_inode))
			ll_inode_lock(inode);
		ktget(&localclock[0]);
		if (vio->vui_iocb->ki_flags & IOCB_DIRECT)
			result = lustre_generic_file_direct_write(vio->vui_iocb, &iter);
		else{
			// result = __generic_file_write_iter(vio->vui_iocb, &iter);
			result = lustre_generic_file_write_iter(vio->vui_iocb, &iter);
		}
		ktget(&localclock[1]);
		ktput(localclock, __generic_file_write_iter);
		if (unlikely(lock_inode))
...
#ifdef HAVE_GENERIC_WRITE_SYNC_2ARGS
		{
			...
			ktget(&localclock[1]);
			ktput(localclock, generic_write_sync);
		}
#else
		{
			ssize_t err;
	...
	if (result > 0) {
		...
		ktget(&localclock[1]);
		ktput(localclock, vvp_io_write_commit);
                              ...
                              const struct cl_io_slice *ios)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = vvp_io_write_start_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_write_start);
	return ret;
}
...
static int vvp_io_fault_start_internal(const struct lu_env *env,
                              const struct cl_io_slice *ios)
{
	struct vvp_io		*vio   = cl2vvp_io(env, ios);
	struct cl_io		*io    = ios->cis_io;
                              ...
                              const struct cl_io_slice *ios)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = vvp_io_fault_start_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_fault_start);
	return ret;
}
...
static int vvp_io_lseek_start_internal(const struct lu_env *env,
			      const struct cl_io_slice *ios)
{
	struct cl_io *io = ios->cis_io;
	struct inode *inode = vvp_object_inode(io->ci_obj);
                              ...
                              const struct cl_io_slice *ios)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = vvp_io_lseek_start_internal(env, ios);
	ktget(&localclock[1]);
	ktput(localclock, vvp_io_lseek_start);
	return ret;
}
