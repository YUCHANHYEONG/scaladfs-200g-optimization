...
// EXPORT_SYMBOL(vm_numa_stat_key); //Kiet
...
DECLARE_PER_CPU(struct work_struct, pcpu_drain);
...
EXPORT_SYMBOL(totalreserve_pages); //Kiet
unsigned long totalcma_pages __read_mostly;

int percpu_pagelist_fraction;
gfp_t gfp_allowed_mask __read_mostly = GFP_BOOT_MASK;
EXPORT_SYMBOL(gfp_allowed_mask); //kiet
...
EXPORT_SYMBOL(gfp_pfmemalloc_allowed); //kiet
...
KTDEF(get_page_from_freelist_1);
EXPORT_SYMBOL(get_page_from_freelist_1_clock);

KTDEF(__alloc_pages_direct_compact);
EXPORT_SYMBOL(__alloc_pages_direct_compact_clock);
 
KTDEF(get_page_from_freelist_2);
EXPORT_SYMBOL(get_page_from_freelist_2_clock);

KTDEF(__alloc_pages_direct_reclaim);
EXPORT_SYMBOL(__alloc_pages_direct_reclaim_clock);

static inline struct page *
__lustre_alloc_pages_slowpath(gfp_t gfp_mask, unsigned int order,
...
retry_cpuset:
	...
	page = get_page_from_freelist(gfp_mask, order, alloc_flags, ac);
	ktget(&localclock[1]);
	ktput(localclock, get_page_from_freelist_1);
	if (page)
		goto got_pg;
	...
	if (can_direct_reclaim &&
		...
		page = __alloc_pages_direct_compact(gfp_mask, order,
						...
						&compact_result);
		ktget(&localclock[1]);
		ktput(localclock, __alloc_pages_direct_compact);
		if (page)
			goto got_pg;
...
retry:
	...
	page = get_page_from_freelist(gfp_mask, order, alloc_flags, ac);
	ktget(&localclock[1]);
	ktput(localclock, get_page_from_freelist_2);
	if (page)
		goto got_pg;
	...
	page = __alloc_pages_direct_reclaim(gfp_mask, order, alloc_flags, ac,
							&did_some_progress);
	ktget(&localclock[1]);
	ktput(localclock, __alloc_pages_direct_reclaim);
	if (page)
		goto got_pg;
...
EXPORT_SYMBOL(__alloc_pages_slowpath_clock);

KTDEF(get_page_from_freelist);
EXPORT_SYMBOL(get_page_from_freelist_clock);

KTDEF(prepare_alloc_pages);
EXPORT_SYMBOL(prepare_alloc_pages_clock); 

/*
 * This is the 'heart' of the zoned buddy allocator.
 */
struct page *
__lustre_alloc_pages_nodemask(gfp_t gfp_mask, unsigned int order, int preferred_nid,
							nodemask_t *nodemask)
{
	struct page *page;
	...
	if (!prepare_alloc_pages(gfp_mask, order, preferred_nid, nodemask, &ac, &alloc_mask, &alloc_flags)){
		ktget(&localclock[1]);
		ktput(localclock, prepare_alloc_pages);
		return NULL;
	}
	ktget(&localclock[1]);
	ktput(localclock, prepare_alloc_pages);
	...
	ktget(&localclock[1]);
	ktput(localclock, get_page_from_freelist);
	if (likely(page))
		goto out;
	...
	ktget(&localclock[0]);
	page = __lustre_alloc_pages_slowpath(alloc_mask, order, &ac);
	//page = __alloc_pages_slowpath(alloc_mask, order, &ac);
	ktget(&localclock[1]);
	ktput(localclock, __alloc_pages_slowpath);
