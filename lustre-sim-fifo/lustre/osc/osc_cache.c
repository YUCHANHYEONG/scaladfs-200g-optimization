...
KTDEF(osc_extent_alloc);
EXPORT_SYMBOL(osc_extent_alloc_clock);
KTDEF(ldlm_lock_get);
EXPORT_SYMBOL(ldlm_lock_get_clock);
KTDEF(osc_extent_search);
EXPORT_SYMBOL(osc_extent_search_clock);
					  ...
					  unsigned int *grants)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_alloc);
	if (cur == NULL)
...
restart:
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_search);
...
KTDEF(osc_extent_finish_top);
EXPORT_SYMBOL(osc_extent_finish_top_clock);

KTDEF(osc_extent_finish_bottom);
EXPORT_SYMBOL(osc_extent_finish_bottom_clock);
/**
 * Called when IO is finished to an extent.
 */
int osc_extent_finish_cal(const struct lu_env *env, struct osc_extent *ext,
		      int sent, int rc)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_finish_top);
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_finish_bottom);
...
KTDEF(osc_extent_make_ready_top);
EXPORT_SYMBOL(osc_extent_make_ready_top_clock);

KTDEF(osc_extent_make_ready_bottom);
EXPORT_SYMBOL(osc_extent_make_ready_bottom_clock);
/**
 ...
 */
static int osc_extent_make_ready_cal(const struct lu_env *env,
				 struct osc_extent *ext)
{
	struct osc_async_page *oap;
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_make_ready_top);
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_extent_make_ready_bottom);
...
#else
#define OSC_DUMP_GRANT(mask, cli, fmt, args...) (void)(0)
#endif
...
KTDEF(cl_loi_list_lock_osc_unreserve_grant);
EXPORT_SYMBOL(cl_loi_list_lock_osc_unreserve_grant_clock);
...
EXPORT_SYMBOL(cl_loi_list_lock_osc_free_grant_clock);
			   ...
			   unsigned int lost_grant, unsigned int dirty_grant)
{
	...
	__this_cpu_sub(obd_dirty_pages_cpu, nr_pages);
...
KTDEF(cl_loi_list_lock_osc_exit_cache);
EXPORT_SYMBOL(cl_loi_list_lock_osc_exit_cache_clock);
			       ...
			       int bytes)
{
	...
	if (cli->cl_dirty_pages < cli->cl_dirty_max_pages) {
		long this_cpu_dirty_pages;
		ktget(&localclock[0]);
		// if (atomic_long_add_return(1, &obd_dirty_pages) <=
		// obd_max_dirty_pages) {
		// pr_info("obd_dirty_pages_cpu = %lu\n", __this_cpu_read(obd_dirty_pages_cpu));
		this_cpu_dirty_pages = __this_cpu_inc_return(obd_dirty_pages_cpu);
		if (this_cpu_dirty_pages <= (long)obd_max_dirty_pages) {
			ktget(&localclock[1]);
			ktput(localclock, osc_enter_cache_atomic);
...
static inline void cli_unlock_and_unplug_direct(const struct lu_env *env,
					 struct client_obd *cli,
					 struct osc_async_page *oap)
{
	spin_unlock(&cli->cl_loi_list_lock);
...
int application_check_req(struct lu_env *env, struct ptlrpc_request_set *set);
KTDEF(application_check_req);
EXPORT_SYMBOL(application_check_req_clock);

KTDEF(application_io_unplug);
EXPORT_SYMBOL(application_io_unplug_clock);

static inline void application_unlock_and_unplug(const struct lu_env *env,
					 struct client_obd *cli,
					 struct osc_async_page *oap,
...
//	ktime_t localclock[2];
	...
	ret = lu_context_init(&temp_env.le_ctx, LCT_MD_THREAD |
					...
					LCT_NOREF);
	if (ret)
		pr_info("lu_context_init failed thread\n");
	ret = lu_context_init(temp_env.le_ses, LCT_SESSION  |
					LCT_REMEMBER |
					LCT_NOREF);
	if (ret)
		pr_info("lu_context_init failed session \n");

	application_io_unplug(&temp_env, cli, NULL, set);
	application_check_req(&temp_env, set);
	lu_context_fini(&temp_env.le_ctx);
	lu_context_fini(temp_env.le_ses);
	CDEBUG(D_CACHE,
	       "%s: sleeping for cache space for %p\n",
	       cli_name(cli), oap);
}

KTDEF(cl_loi_list_lock_cli_lock_after_unplug);
EXPORT_SYMBOL(cl_loi_list_lock_cli_lock_after_unplug_clock);
...
KTDEF(osc_enter_cache_wait);
EXPORT_SYMBOL(osc_enter_cache_wait_clock);
 ...
 */
KTDEF(ptlrpc_prep_set);
EXPORT_SYMBOL(ptlrpc_prep_set_clock);

KTDEF(ych_cl_loi_list_lock);
EXPORT_SYMBOL(ych_cl_loi_list_lock_clock);

KTDEF(ych_osc_enter_cache_try_1);
EXPORT_SYMBOL(ych_osc_enter_cache_try_1_clock);

KTDEF(application_unlock_and_unplug);
EXPORT_SYMBOL(application_unlock_and_unplug_clock);

KTDEF(cli_lock_after_unplug);
EXPORT_SYMBOL(cli_lock_after_unplug_clock);

KTDEF(ych_osc_enter_cache_try_2);
EXPORT_SYMBOL(ych_osc_enter_cache_try_2_clock);

KTDEF(ptlrpc_set_destroy);
EXPORT_SYMBOL(ptlrpc_set_destroy_clock);
...
//	ktput(localclock, osc_enter_cache_wait);
	...
	if (entered == 0){
		...
		application_unlock_and_unplug(env, cli, oap, set);
		ktget(&localclock[1]);
		ktput(localclock, application_unlock_and_unplug);
	...
	if (entered) {
		...
		pr_info("osc_enter_cache_wait: timeout, fall back to sync i/o\n");
		OSC_DUMP_GRANT(D_CACHE, cli,
			       "timeout, fall back to sync i/o\n");
		osc_extent_tree_dump(D_CACHE, osc);
		/* fall back to synchronous I/O */
	} else {
		// pr_info("osc_enter_cache_wait: no grant space, fall back to sync i/o\n");
		OSC_DUMP_GRANT(D_CACHE, cli,
...
out:
	...
	if  (set != NULL){
		...
		ktget(&localclock[1]);
		ktput(localclock, ptlrpc_set_destroy);
	}
	/* ych add	*/
	RETURN(rc);
}
...
static int osc_enter_cache_old(const struct lu_env *env, struct client_obd *cli,
			   struct osc_async_page *oap, int bytes)
{
	struct osc_object *osc = oap->oap_obj;
	struct lov_oinfo *loi = osc->oo_oinfo;
...
static int application_makes_rpc(struct client_obd *cli, struct osc_object *osc,
			 int cmd)
{
	int invalid_import = 0;
	ENTRY;

	// pr_info("application makes rpc \n");
...
KTDEF(cl_loi_list_lock_osc_list_maint);
EXPORT_SYMBOL(cl_loi_list_lock_osc_list_maint_clock);
...
static int osc_list_maint(struct client_obd *cli, struct osc_object *osc)
{
	...
	spin_lock(&cli->cl_loi_list_lock);
	ktget(&localclock[1]);
	ktput(localclock, cl_loi_list_lock_osc_list_maint);
...
KTDEF(cl_loi_list_lock_osc_ap_completion);
EXPORT_SYMBOL(cl_loi_list_lock_osc_ap_completion_clock);
			      ...
			      struct osc_async_page *oap, int sent, int rc)
{
	...
	if (oap->oap_cmd & OBD_BRW_WRITE && xid > 0) {
		...
		spin_lock(&cli->cl_loi_list_lock);
		ktget(&localclock[1]);
		ktput(localclock, cl_loi_list_lock_osc_ap_completion);
...
static int
application_send_write_rpc(const struct lu_env *env, struct client_obd *cli,
		   struct osc_object *osc, struct ptlrpc_request_set *set)
__must_hold(osc)
{
	LIST_HEAD(rpclist);
	struct osc_extent *ext;
	...
	// pr_info("application_send_write_rpc\n");
	...
	if (!list_empty(&rpclist)) {
		LASSERT(page_count > 0);
		rc = application_build_rpc(env, cli, &rpclist, OBD_BRW_WRITE, set);
		LASSERT(list_empty(&rpclist));
	}
...
static struct osc_object *application_osc_next_obj(struct client_obd *cli)
{
	ENTRY;
	 ...
	 */
	if (!list_empty(&cli->cl_loi_write_list)){
		// pr_info("osc_object from the cli->cl_loi_write_list\n");
		RETURN(list_to_obj(&cli->cl_loi_write_list, write_item));
	}
...
KTDEF(cl_loi_list_lock_osc_check_rpcs);
EXPORT_SYMBOL(cl_loi_list_lock_osc_check_rpcs_clock);
...
__must_hold(&cli->cl_loi_list_lock)
{
	...
	while ((osc = osc_next_obj(cli)) != NULL) {
		...
		spin_lock(&cli->cl_loi_list_lock);
		ktget(&localclock[1]);
		ktput(localclock, cl_loi_list_lock_osc_check_rpcs);
	}
	EXIT;
}
...
__must_hold(&cli->cl_loi_list_lock)
{
	...
	while ((osc = application_osc_next_obj(cli)) != NULL) {
		struct cl_object *obj = osc2cl(osc);
		struct lu_ref_link link;

		OSC_IO_DEBUG(osc, "%lu in flight\n", rpcs_in_flight(cli));
		// pr_info("application_check_rpcs\n");
		/* even if we have reached our max in flight RPCs, we still
		 * allow all high-priority RPCs through to prevent their
		 * starvation and leading to server evicting us for not
		 * writing out pages in a timely manner LU-13131 */
		 ...
		 * do io on writes while there are cache waiters */
		osc_object_lock(osc);
		if (application_makes_rpc(cli, osc, OBD_BRW_WRITE)) {
			// pr_info("Application makes write rpc\n");
			rc = application_send_write_rpc(env, cli, osc, set);
			if (rc < 0) {
				CERROR("Write request failed with %d\n", rc);
				pr_info("Write request failed with %d\n", rc);
		...
		if (application_makes_rpc(cli, osc, OBD_BRW_READ)) {
			rc = osc_send_read_rpc(env, cli, osc);
			if (rc < 0)
				CERROR("Read request failed with %d\n", rc);
		}
		...
		spin_lock(&cli->cl_loi_list_lock);
		ktget(&localclock[1]);
		ktput(localclock, cl_loi_list_lock_application_check_rpcs);
	}
	EXIT;
}

KTDEF(osc_io_unplug0_sync);
EXPORT_SYMBOL(osc_io_unplug0_sync_clock);
		   ...
		   struct osc_object *osc, int async)
{
	...
	if (!async) {
		...
		spin_lock(&cli->cl_loi_list_lock);
		ktget(&localclock[1]);
		ktput(localclock, osc_io_unplug0_sync);
		osc_check_rpcs(env, cli);
		spin_unlock(&cli->cl_loi_list_lock);
	} else {
		CDEBUG(D_CACHE, "Queue writeback work for client %p.\n", cli);
		// pr_info("Queue writeback work for client %p.\n", cli);
...
KTDEF(cl_loi_list_lock_application_io_unplug);
EXPORT_SYMBOL(cl_loi_list_lock_application_io_unplug_clock);

int application_io_unplug(const struct lu_env *env, struct client_obd *cli,
		   struct osc_object *osc, struct ptlrpc_request_set *set)
{
	int rc = 0;
	ktime_t localclock[2];
	...
	spin_lock(&cli->cl_loi_list_lock);
	ktget(&localclock[1]);
	ktput(localclock, cl_loi_list_lock_application_io_unplug);
	application_check_rpcs(env, cli, set);
	spin_unlock(&cli->cl_loi_list_lock);
	return rc;
}
EXPORT_SYMBOL(application_io_unplug);
...
KTDEF(cl_loi_list_lock_osc_queue_async_io_1);
EXPORT_SYMBOL(cl_loi_list_lock_osc_queue_async_io_1_clock);

KTDEF(cl_loi_list_lock);
EXPORT_SYMBOL(cl_loi_list_lock_clock);

KTDEF(osc_enter_cache);
EXPORT_SYMBOL(osc_enter_cache_clock);

KTDEF(osc_extent_release);
EXPORT_SYMBOL(osc_extent_release_clock);

KTDEF(osc_enter_cache_try);
EXPORT_SYMBOL(osc_enter_cache_try_clock);

KTDEF(osc_extent_find);
EXPORT_SYMBOL(osc_extent_find_clock);

/* taehwan code */
atomic64_t total_grant_result[3];

static DEFINE_PER_CPU(unsigned long [4], percore_result);
/* taehwan code */
		       ...
		       struct osc_page *ops, cl_commit_cbt cb)
{
	...
	/* taehwan code */
	int max_idx;
	unsigned long max_value, tmp_value;

	int cpu, i;
	size_t target_bs_size = io->u.ci_rw.crw_count;
	unsigned long *core_result = this_cpu_ptr(percore_result);
	core_result[0]++;
	if (target_bs_size <= (1024*16)) {
		core_result[1]++;
	} else if (target_bs_size <= (1024*64)) {
		core_result[2]++;
	} else {
		core_result[3]++;
	}
	
	if (core_result[0] >= 1000) {
		core_result[0]=0;
		for (i = 0; i < 3; i++) {
			atomic64_set(&total_grant_result[i], 0);
		}
		for_each_online_cpu(cpu) {
			core_result = per_cpu_ptr(percore_result, cpu);
			for (i = 0; i < 3; i++) {
				atomic64_add(core_result[i+1], &total_grant_result[i]);
				core_result[i+1]=0;
			}
		}
		
		max_idx = 0;
		max_value = atomic64_read(&total_grant_result[0]);
		for (i = 1; i < 3; i++) {
			tmp_value = atomic64_read(&total_grant_result[i]);
		...
		if (max_idx == 0) {
			cli->cl_avail_grant = (unsigned long)256 * 1024 * 1024 * 1024;
		} else if (max_idx == 1) {
			cli->cl_avail_grant = (unsigned long)16 * 1024 * 1024 * 1024;
		} else if (max_idx == 2) {
	...
	if (ext != NULL && ext->oe_start <= index && ext->oe_max_end >= index) {
		...
		ktget(&localclock[1]);
		ktput(localclock, osc_enter_cache_try);
	...
	if (need_release) {
		...
		osc_extent_release(env, ext);
		ktget(&localclock[1]);
		ktput(localclock, osc_extent_release);
		oio->oi_active = NULL;
		ext = NULL;
	}
	...
	if (ext == NULL) {
		...
		if (grants == 0) {
			...
			ktget(&localclock[1]);
			ktput(localclock, osc_enter_cache);
			if (rc == 0)
				grants = tmp;
		}
		...
		if (rc == 0) {
			...
			ktget(&localclock[1]);
			ktput(localclock, osc_extent_find);
			if (IS_ERR(ext)) {
				LASSERT(tmp == grants);
...
KTDEF(cl_loi_list_lock_osc_queue_sync_pages);
EXPORT_SYMBOL(cl_loi_list_lock_osc_queue_sync_pages_clock);
			 ...
			 int brw_flags)
{
	...
	if (ext->oe_dio && !ext->oe_rw) { /* direct io write */
		...
		if (osc_reserve_grant(cli, grants) == 0) {
			...
			__this_cpu_add(obd_dirty_pages_cpu, page_count);
	...
	if (ext->oe_is_rdma_only)
		pr_info("osc_queue_sync_pages: ext->oe_is_rdma_only = %d\n", ext->oe_is_rdma_only);
	...
	// if (ret)
	// 	pr_info("lu_context_init failed session \n");
	// application_io_unplug(&temp_env, cli, obj, set);
	// application_check_req(&temp_env, set);
	// lu_context_fini(&temp_env.le_ctx);
	// lu_context_fini(temp_env.le_ses);
...
KTDEF(osc_io_unplug);
EXPORT_SYMBOL(osc_io_unplug_clock);

KTDEF(osc_cache_while);
EXPORT_SYMBOL(osc_cache_while_clock);

KTDEF(osc_cache_bottom);
EXPORT_SYMBOL(osc_cache_bottom_clock);
 ...
 */
int fsync_osc_cache_writeback_range(const struct lu_env *env, struct osc_object *obj,
			      pgoff_t start, pgoff_t end, int hp, int discard)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_io_unplug);
	if (ext == NULL){
	...
	while (ext != NULL) {
		if (ext->oe_start > end){
			//printk("[%s] pid=%d ych_3\n", __func__, current->pid);
			break;
		}

		//printk("[%s] pid=%d ych_4\n", __func__, current->pid);
		ext->oe_fsync_wait = 1;
		switch (ext->oe_state) {
		case OES_CACHE:
			//printk("[%s] pid=%d ych_5\n", __func__, current->pid);
			result += ext->oe_nr_pages;
			if (!discard) {
				struct list_head *list = NULL;
				//printk("[%s] pid=%d ych_6\n", __func__, current->pid);
				if (hp) {
					EASSERT(!ext->oe_hp, ext);
					//printk("[%s] pid=%d ych_7\n", __func__, current->pid);
					ext->oe_hp = 1;
					list = &obj->oo_hp_exts;
				} else if (!ext->oe_urgent && !ext->oe_hp) {
					//printk("[%s] pid=%d ych_8\n", __func__, current->pid);
					ext->oe_urgent = 1;
					list = &obj->oo_urgent_exts;
				}
				if (list != NULL){
					//printk("[%s] pid=%d ych_9\n", __func__, current->pid);
					list_move_tail(&ext->oe_link, list);
				}
				//printk("[%s] pid=%d ych_10\n", __func__, current->pid);
				unplug = true;
			} else {
				struct client_obd *cli = osc_cli(obj);
	...
	if (!list_empty(&discard_list)) {
		...
		list_for_each_entry_safe(ext, tmp, &discard_list, oe_link) {
			...
			ktget(&localclock[0]);
			rc = osc_extent_make_ready_cal(env, ext);
			ktget(&localclock[1]);
			ktput(localclock, osc_cache_while);
			if (unlikely(rc < 0)) {
				OSC_EXTENT_DUMP(D_ERROR, ext,
			...
			ktget(&localclock[0]);
			osc_extent_finish_cal(env, ext, 0, 0);
			ktget(&localclock[1]);
			ktput(localclock, osc_cache_bottom);
		}
	}


	if (unplug){
		//printk("[%s] pid=%d ych_11\n", __func__, current->pid);
		//printk("[%s] pid=%d start!\n", __func__, current->pid);
		osc_io_unplug(env, osc_cli(obj), obj);
		//printk("[%s] pid=%d end!\n", __func__, current->pid);
	}
...
EXPORT_SYMBOL(fsync_osc_cache_writeback_range);
