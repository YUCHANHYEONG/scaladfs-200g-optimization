	...
	if (ldlm_is_granted(dlmlock)) {
		...
		if (dlmlock->l_granted_mode == LCK_PW) {
			...
			if (atomic_cmpxchg(&osc->oo_pw_replacing, 0, 1) == 0) {
				 ...
				 */
				if (old == NULL) {
					if (atomic_cmpxchg(&osc->oo_pw_state,
						 ...
						 * by REPLACING.
						 */
						osc->oo_cached_pw_lock = new_lock;

						atomic_set(&osc->oo_pw_full_range,
								ext->start == 0 &&
						...
						if (state != OSC_PW_CACHE_REPLACING) {
							 ...
							 */
							if (osc->oo_cached_pw_lock == new_lock)
				 ...
				 */
					...
					if (new_ext->start <= old_ext->start &&
						...
						if (atomic_read(&osc->oo_fast_users) == 0) {
							...
							old_full_range = atomic_read(&osc->oo_pw_full_range);
							...
							if (state ==
								 ...
								 */
								osc->oo_cached_pw_lock = old;
								atomic_set(&osc->oo_pw_full_range, old_full_range);
								LDLM_LOCK_PUT(new_lock);
							}
						} else {
							/*
                                 ...
                                 int flag)
{
	...
	case LDLM_CB_BLOCKING: {
		...
		if (osc != NULL) {
			int state;
retry:
			state = atomic_read(&osc->oo_pw_state);
			/*
...
KTDEF(osc_enqueue_base);
EXPORT_SYMBOL(osc_enqueue_base_clock);
 ...
 */
static int osc_lock_enqueue_internal(const struct lu_env *env,
...
enqueue_base:
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_enqueue_base);
	if (result == 0) {
		if (osc_lock_is_lockless(oscl)) {
			oio->oi_lockless = 1;
		} else if (!async) {
			    ...
			    struct cl_io *unused, struct cl_sync_io *anchor)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = osc_lock_enqueue_internal(env, slice, unused, anchor);
	ktget(&localclock[1]);
	ktput(localclock, osc_lock_enqueue);
	return ret;
}
