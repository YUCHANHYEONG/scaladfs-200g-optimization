                                ...
                                long writing_bytes)
{
	...
	} else if (unlikely(__this_cpu_read(obd_dirty_pages_cpu) >
...
void osc_init_grant(struct client_obd *cli, struct obd_connect_data *ocd)
{
	...
	//cli->cl_avail_grant = 8UL * 1024 * 1024;			// 8MB
	//cli->cl_avail_grant = 512UL * 1024 * 1024;			// 512MB
	//cli->cl_avail_grant = 1UL * 1024 * 1024 * 1024;		// 1GB
	//cli->cl_avail_grant = 2UL * 1024 * 1024 * 1024;		// 2GB
	//cli->cl_avail_grant = 4UL * 1024 * 1024 * 1024;		// 4GB
	//cli->cl_avail_grant = 8UL * 1024 * 1024 * 1024;		// 8GB
	//cli->cl_avail_grant = 16UL * 1024 * 1024 * 1024;		// 16GB
	//cli->cl_avail_grant = 32UL * 1024 * 1024 * 1024;		// 32GB
	//cli->cl_avail_grant = 64UL * 1024 * 1024 * 1024;		// 64GB
	//cli->cl_avail_grant = 128UL * 1024 * 1024 * 1024;		// 128GB
	//cli->cl_avail_grant = 256UL * 1024 * 1024 * 1024;		// 256GB
	//cli->cl_avail_grant = 512UL * 1024 * 1024 * 1024;		// 512GB
	//cli->cl_avail_grant = 1UL * 1024 * 1024 * 1024 * 1024;	// 1TB
	//cli->cl_avail_grant = 4400UL * 1024 * 1024 * 1024;		// 4.4TB
	//cli->cl_avail_grant = cli->cl_dirty_max_pages * 4 * 1024 * 8;
	//cli->cl_avail_grant = cli->cl_dirty_max_pages * 4 * 1024 * 1024; //current best
	//cli->cl_avail_grant = cli->cl_dirty_max_pages * 4 * 1024 * 1024 * 4;
	//cli->cl_avail_grant = cli->cl_dirty_max_pages * 4 * 1024;
	//pr_info("cli->cl_avail_grant: %ld\n", cli->cl_avail_grant);
	
	// Dirty Page Size
	//cli->cl_dirty_max_pages = 67108864UL / 2;
	//cli->cl_dirty_max_pages = cli->cl_dirty_max_pages * 4;
	//cli->cl_dirty_max_pages = 256000UL;		// 1GB
	//cli->cl_dirty_max_pages = 256000UL * 2;	// 2GB
...
retry_encrypt:
	...
	// pr_info("cli->cl_import->imp_connect_data.ocd_brw_size = %d\n", cli->cl_import->imp_connect_data.ocd_brw_size);
	...
	desc = ptlrpc_prep_bulk_imp_multiple_md(req, page_count,
		cli->cl_import->imp_connect_data.ocd_brw_size >> LNET_MTU_BITS,
		(opc == OST_WRITE ? PTLRPC_BULK_GET_SOURCE :
			PTLRPC_BULK_PUT_SINK),
		OST_BULK_PORTAL,
		&ptlrpc_bulk_kiov_multiple_md_ops);

        if (desc == NULL){
			pr_info("desc is NULL\n");
                GOTO(out, rc = -ENOMEM);
		}
	...
	pg_prev = pga[0];
		...
		if (short_io_size != 0 && opc == OST_WRITE) {
			...
			desc->bd_frag_ops->add_kiov_frag(desc, pg->pg, poff,
							 pg->count);
			// pr_info("Function pointer %pS\n", desc->bd_frag_ops->add_kiov_frag);

			// if (desc->bd_is_rdma) //check is_irdma flag in desc
			// 	pr_info("is_irdma \n");
		}
...
static int osc_brw_fini_request(struct ptlrpc_request *req, int rc)
{
	...
	if (!(aa->aa_oa)){
		pr_info("aa_oa is NULL: request %px aa %px rc %d\n", req, aa, rc);
		pr_info("Null and req->application is: %d\n", atomic_read(&req->application));
		dump_stack();
		return -1;
	}
			 ...
			 struct ptlrpc_request *req, void *args, int rc)
{
	...
	if (!req){
		pr_info("request is NULL aa %p rc %d\n", aa, rc);
		dump_stack();
		BUG_ON(!req);
	}
	ktget(&localclock[0]);
	...
	ktget(&localclock[1]);
	ktput(localclock, osc_brw_fini_request);
	if (rc == -1)
		goto free;
...
KTDEF(brw_interpret);
EXPORT_SYMBOL(brw_interpret_clock);
			 ...
			 struct ptlrpc_request *req, void *args, int rc)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = brw_interpret_internal(env, req, args, rc);
	ktget(&localclock[1]);
	ktput(localclock, brw_interpret);
	return ret;
}
...
KTDEF(osc_set_lock_data_wrap);
EXPORT_SYMBOL(osc_set_lock_data_wrap_clock);

static int osc_set_lock_data_wrap(struct ldlm_lock *lock, void *data)
{
    int ret;
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = osc_set_lock_data(lock, data);
	ktget(&localclock[1]);
	ktput(localclock, osc_set_lock_data_wrap);
	return ret;
}
...
KTDEF(ldlm_lock_match_with_skip);
EXPORT_SYMBOL(ldlm_lock_match_with_skip_clock);

KTDEF(ldlm_cli_enqueue);
EXPORT_SYMBOL(ldlm_cli_enqueue_clock);

KTDEF(osc_lock_upcall);
EXPORT_SYMBOL(osc_lock_upcall_clock);
		     ...
		     bool speculative)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, ldlm_lock_match_with_skip);
	...
	if (mode) {
		...
		if (speculative) {
			...
			RETURN(rc);
		} else if (osc_set_lock_data_wrap(matched, einfo->ei_cbdata)) {
			*flags |= LDLM_FL_LVB_READY;
	...
	if (*flags & (LDLM_FL_TEST_LOCK | LDLM_FL_MATCH_LOCK))
        ...
        *flags &= ~LDLM_FL_BLOCK_GRANTED;
		...
		ktget(&localclock[1]);
		ktput(localclock, ldlm_cli_enqueue);
	if (async) {
		if (!rc) {
