...
static inline struct inode *lf_wb_inode(struct list_head *head)
{
	return lflist_entry(head, struct inode, i_io_list);
}
...
atomic_t dirty_atomic = ATOMIC_INIT(0);
...
LFLIST_HEAD(lf_b_more_io);
EXPORT_SYMBOL(lf_b_more_io);

static void wb_io_lists_depopulated(struct bdi_writeback *wb) //may need to update bcuz of non-empty local list
{
	if (wb_has_dirty_io(wb) && list_empty(&wb->test_list) &&
	    list_empty(&wb->b_io) && list_empty(&wb->b_more_io)
		&& !atomic_read(&dirty_atomic))
	{
...
KTDEF(InsertInode);
EXPORT_SYMBOL(InsertInode_clock);
...
// static bool inode_io_list_move_nolocked(struct inode *inode,
// 				      struct bdi_writeback *wb)
// {
...
static void lustre_inode_cgwb_move_to_attached(struct inode *inode,
					struct bdi_writeback *wb)
{
	// assert_spin_locked(&wb->list_lock);
	assert_spin_locked(&inode->i_lock);
				...
				struct bdi_writeback *new_wb)
{
	 ...
	 * ->test_list which is always correct including from ->b_dirty_time.
	...
	if (!list_empty(&inode->i_io_list)) {
		...
		if (inode->i_state & I_DIRTY_ALL) {
			...
			list_for_each_entry(pos, &new_wb->test_list, i_io_list)
 ...
 */
void lustre_wbc_attach_and_unlock_inode(struct writeback_control *wbc,
				 struct inode *inode)
{
	if (!inode_cgwb_enabled(inode)) {
		spin_unlock(&inode->i_lock);
...
static void l_inode_cgwb_move_to_attached(struct inode *inode,
					struct bdi_writeback *wb)
{
	assert_spin_locked(&wb->list_lock);
	assert_spin_locked(&inode->i_lock);
	...
	inode->i_state &= ~I_SYNC_QUEUED;
	if (&inode->i_io_list == inode->i_io_list.prev.next)
		list_del_init(&inode->i_io_list);
	else{
		/// ????? How to solve?????
...
EXPORT_SYMBOL(lustre_inode_io_list_del);
 ...
 * already the most-recently-dirtied inode on the test_list list.  If that is
 * the case then the inode must have been redirtied while it was being written
 * out and we don't reset its dirtied_when.
 */
static void redirty_tail_locked_prealloc(struct inode *inode, struct bdi_writeback *wb, struct LNode *lnode)
{
	//printk("[%s] start! from %ps\n", __func__, __builtin_return_address(0));
	assert_spin_locked(&inode->i_lock);
	inode->dirtied_when = jiffies;
	inode_io_list_move_locked_prealloc(inode, wb, &wb->test_list, lnode);
	inode->i_state &= ~I_SYNC_QUEUED;
}
...
static void redirty_tail_locked(struct inode *inode, struct bdi_writeback *wb)
{
	//printk("[%s] start! from %ps\n", __func__, __builtin_return_address(0));
	assert_spin_locked(&inode->i_lock);
	// assert_spin_locked(&wb->list_lock); // for lock-free check

	// if (!list_empty(&wb->test_list)) {
	// 	struct inode *tail;

	// 	tail = wb_inode(wb->test_list.next);
	// 	if (time_before(inode->dirtied_when, tail->dirtied_when))
	// 		inode->dirtied_when = jiffies;
	// }
	inode->dirtied_when = jiffies;
	inode_io_list_move_locked(inode, wb, &wb->test_list);
	inode->i_state &= ~I_SYNC_QUEUED;
}
 ...
 * Before
 *         newly dirtied     test_list    b_io    b_more_io
 *         =============>    gf         edc     BA
 * After
 *         newly dirtied     test_list    b_io    b_more_io
 *         =============>    g          fBAedc
		     ...
		     unsigned long dirtied_before)
{
	...
	moved = move_expired_inodes(&wb->test_list, &wb->b_io, dirtied_before);
	if (!work->for_sync)
...
/* Supposedly return the "oldest" dirtied inode that dirtied before dirtied_before
	It could have problem if the lockfree operation changes the order, the last inode in the list may be earlier dirtied that its previous inode.

*/

static struct inode *lustre_queue_io(struct bdi_writeback *wb, struct wb_writeback_work *work,
		     unsigned long dirtied_before)
{
	int moved = 0;
	// unsigned long time_expire_jif = dirtied_before;
	...
	// assert_spin_locked(&wb->list_lock);
	list = FetchHead(&wb->lock_free_dirty);
...
static inline void lustre_mark_inode_dirty_sync(struct inode *inode)
{
	__lustre_mark_inode_dirty(inode, I_DIRTY_SYNC);
}
 ...
 * potentially to move it from b_dirty_time to test_list due to timestamp
...
__writeback_single_inode(struct inode *inode, struct writeback_control *wbc)
{
	...
	ret = lustre_do_writepages(mapping, wbc);
	...
	if ((inode->i_state & I_DIRTY_TIME) &&
		...
		lustre_mark_inode_dirty_sync(inode);
	}
...
KTDEF(__writeback_single_inode);
EXPORT_SYMBOL(__writeback_single_inode_clock);

KTDEF(inode_to_wb_and_lock_list);
EXPORT_SYMBOL(inode_to_wb_and_lock_list_clock);
...
EXPORT_SYMBOL(__writeback_inodes_wb_clock);
			 ...
			 struct wb_writeback_work *work)
{
	...
	for (;;) {
		 ...
		 */
		if (work->for_background && !lustre_wb_over_bg_thresh(wb))
			break;
...
/*
	Remove one expired inode from the list and do the writeback.
*/
KTDEF(lustre_wb_writeback_spinlock);
EXPORT_SYMBOL(lustre_wb_writeback_spinlock_clock);


atomic_t debug_value = ATOMIC_INIT(0);


static long lustre_wb_writeback(struct bdi_writeback *wb,
			 struct wb_writeback_work *work)
{
	long nr_pages = work->nr_pages;
	unsigned long dirtied_before = jiffies;
	struct inode *inode = NULL; // inode that we want to writeback
	long progress;
	struct blk_plug plug;
	...
	for (;;) {
		 ...
		 * Stop writeback when nr_pages has been consumed
		 */
		// BUG_ON(++counter == ULLONG_MAX);

		// if (counter > 1)
		// 	pr_info("[%d] Stuck in the loop lustre_wb_writeback, counter: %llu\n", current->pid, counter);
		if (work->nr_pages <= 0){
			break;
		}
		...
		// spin_lock(&wb->list_lock);
		// ktget(&localclock[1]);
		// ktput(localclock, lustre_wb_writeback_spinlock);
		...
		inode = lustre_queue_io(wb, work, dirtied_before);

		if (!inode){
			// pr_info("Empty dirty list \n");
			break;
		}
		// else
		// 	pr_info("Inode ok \n");
		ktget(&localclock[0]);
		progress = __lustre_writeback_single(wb, inode, work);
		ktget(&localclock[1]);
		ktput(localclock, __writeback_inodes_wb);
	...
	// spin_unlock(&wb->list_lock);
	//pr_info("Done the writeback \n")
	atomic_dec(&dirty_atomic);
	blk_finish_plug(&plug);

	// if (counter > 0)
	// 	pr_info("[%d] %s done, counter: %llu\n", current->pid, __func__, counter);
...
KTDEF(wb_over_bg_thresh);
EXPORT_SYMBOL(wb_over_bg_thresh_clock);
...
static long wb_check_background_flush(struct bdi_writeback *wb)
{
	...
	bool need_writeback;
	ktget(&localclock[0]);
	need_writeback = lustre_wb_over_bg_thresh(wb);
	// if (need_writeback)
	// 	if (printk_ratelimit())
	// 		pr_info("Need writeback\n");
	ktget(&localclock[1]);
	ktput(localclock, wb_over_bg_thresh);
	// if (wb_over_bg_thresh(wb)) {
	if (need_writeback){
		struct wb_writeback_work work = {
			.nr_pages	= LONG_MAX,
...
static long wb_check_start_all(struct bdi_writeback *wb)
{
	...
	if (nr_pages) {
		...
		nr_pages = lustre_wb_writeback(wb, &work);
	}
...
KTDEF(wb_check_background_flush);
EXPORT_SYMBOL(wb_check_background_flush_clock);

KTDEF(set_bit_wb);
EXPORT_SYMBOL(set_bit_wb_clock);

/*
 * Retrieve work items and do the writeback they describe
 */
long lustre_wb_do_writeback(struct bdi_writeback *wb)
{
	struct wb_writeback_work *work;
	long wrote = 0;
	ktime_t localclock[2];
	...
	percpu_ref_get(&wb->running_refcnt);
	ktget(&localclock[1]);
	ktput(localclock, set_bit_wb);
if (0) {
	...
	// if (printk_ratelimit())
	// 	pr_info("wb struct pointer %px\n", wb);
	ktget(&localclock[0]);
	wrote += wb_check_background_flush(wb);
	ktget(&localclock[1]);
	ktput(localclock, wb_check_background_flush);
	// clear_bit(WB_writeback_running, &wb->state);
	percpu_ref_put(&wb->running_refcnt);

	return wrote;
}
EXPORT_SYMBOL(lustre_wb_do_writeback);
 ...
 */
KTDEF(lustre_wb_workfn);
EXPORT_SYMBOL(lustre_wb_workfn_clock);
void lustre_wb_workfn(struct work_struct *work)
{
	...
	if (likely(!current_is_workqueue_rescuer() ||
		...
		do {
			ktget(&stopwatch[0]);
			pages_written = lustre_wb_do_writeback(wb);
			ktget(&stopwatch[1]);
			ktput(stopwatch, lustre_wb_workfn);
...
EXPORT_SYMBOL(lustre_wb_workfn);
 ...
 * test_listtime list in wb_has_dirty_io(), since this would cause the
...
//			__func__, raw_smp_processor_id(), current->comm, current->pid);
	...
	if ((inode->i_state & flags) != flags) {
		...
		if (!was_dirty) {
			...
			wakeup_bdi = inode_io_list_move_locked_prealloc(
					inode, wb, dirty_list, lnode);
