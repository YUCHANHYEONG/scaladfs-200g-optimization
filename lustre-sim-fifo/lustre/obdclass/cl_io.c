...
KTDEF(cl_lockset_lock);
EXPORT_SYMBOL(cl_lockset_lock_clock);
...
int cl_io_lock(const struct lu_env *env, struct cl_io *io)
{
	...
	if (result == 0) {
		...
		ktget(&localclock[0]);
		result = cl_lockset_lock(env, io, &io->ci_lockset); //call cl_lock_request
		ktget(&localclock[1]);
		ktput(localclock, cl_lockset_lock);
	}
	if (result != 0)
		cl_io_unlock(env, io);
	else
...
KTDEF(cis_op);
EXPORT_SYMBOL(cis_op_clock);

/**
 * Starts io by calling cl_io_operations::cio_start() top-to-bottom.
 */

int fsync_cl_io_start(const struct lu_env *env, struct cl_io *io)
{
	const struct cl_io_slice *scan;
	int result = 0;
	ktime_t localclock[2];
...
EXPORT_SYMBOL(fsync_cl_io_start);
...
KTDEF(sync_cio_start);
EXPORT_SYMBOL(sync_cio_start_clock);

int fsync_sync_cl_io_start(const struct lu_env *env, struct cl_io *io)
{
	const struct cl_io_slice *scan;
	int result = 0;
	ktime_t localclock[2];
	...
	list_for_each_entry(scan, &io->ci_layers, cis_linkage) {
		if (scan->cis_iop->op[io->ci_type].cio_start == NULL)
			continue;
		//printk("%pS\n", scan->cis_iop->op[io->ci_type].cio_start);
		// vvp_io_fsync_start()
		// lov_io_start()
		// osc_io_fsync_start()
		ktget(&localclock[0]);
		result = scan->cis_iop->op[io->ci_type].cio_start(env, scan);
		ktget(&localclock[1]);
		ktput(localclock, sync_cio_start);
		if (result != 0)
			break;
	}
	if (result >= 0)
...
EXPORT_SYMBOL(fsync_sync_cl_io_start);


int sync_cl_io_start(const struct lu_env *env, struct cl_io *io)
{
	const struct cl_io_slice *scan;
	int result = 0;
	ktime_t localclock[2];
...
EXPORT_SYMBOL(sync_cl_io_start);
...
KTDEF(cl_sync_io_wait_submit_sync);
EXPORT_SYMBOL(cl_sync_io_wait_submit_sync_clock);
		      ...
		      long timeout)
{
	...
	if (rc == 0) {
		...
		ktget(&localclock[1]);
		ktput(localclock, cl_sync_io_wait_submit_sync);
...
KTDEF(sync_cl_io_lock);
KTDEF(sync_cl_io_iter_init);
KTDEF(sync_cl_io_start);
KTDEF(sync_cl_io_end);
KTDEF(sync_cl_io_unlock);
KTDEF(sync_cl_io_iter_fini);
KTDEF(sync_cl_io_rw_advance);
EXPORT_SYMBOL(sync_cl_io_lock_clock);
EXPORT_SYMBOL(sync_cl_io_iter_init_clock);
EXPORT_SYMBOL(sync_cl_io_start_clock);
EXPORT_SYMBOL(sync_cl_io_end_clock);
EXPORT_SYMBOL(sync_cl_io_unlock_clock);
EXPORT_SYMBOL(sync_cl_io_iter_fini_clock);
EXPORT_SYMBOL(sync_cl_io_rw_advance_clock);

int fsync_sync_cl_io_loop(const struct lu_env *env, struct cl_io *io)
{
	int result = 0;
	int rc = 0;
	ktime_t localclock[2];
	...
	do {
		...
		ktget(&localclock[1]);
		ktput(localclock, sync_cl_io_iter_init);
		if (result == 0) {
			nob    = io->ci_nob;
			ktget(&localclock[0]);
			result = cl_io_lock(env, io);
			ktget(&localclock[1]);
			ktput(localclock, sync_cl_io_lock);
			if (result == 0) {
				/*
				 ...
				 */
				ktget(&localclock[0]);
				result = fsync_sync_cl_io_start(env, io);
				//result = sync_cl_io_start(env, io);
				ktget(&localclock[1]);
				ktput(localclock, sync_cl_io_start);
				/*
				...
				cl_io_end(env, io);
				ktget(&localclock[1]);
				ktput(localclock, sync_cl_io_end);

				ktget(&localclock[0]);
				//printk("[%s] pid=%d ych_5\n", __func__, current->pid);
				cl_io_unlock(env, io);
				ktget(&localclock[1]);
				ktput(localclock, sync_cl_io_unlock);

				ktget(&localclock[0]);
				//printk("[%s] pid=%d ych_6\n", __func__, current->pid);
				cl_io_rw_advance(env, io, io->ci_nob - nob);
				ktget(&localclock[1]);
				ktput(localclock, sync_cl_io_rw_advance);
			}
		}
		ktget(&localclock[0]);
		...
		ktget(&localclock[1]);
		ktput(localclock, sync_cl_io_iter_fini);
		if (result)
			rc = result;
...
EXPORT_SYMBOL(fsync_sync_cl_io_loop);

int sync_cl_io_loop(const struct lu_env *env, struct cl_io *io)
{
	int result = 0;
	int rc = 0;
...
EXPORT_SYMBOL(sync_cl_io_loop);

KTDEF(cl_io_lock);
KTDEF(cl_io_iter_init);
KTDEF(cl_io_start);
KTDEF(cl_io_end);
KTDEF(cl_io_unlock);
KTDEF(cl_io_iter_fini);
KTDEF(cl_io_rw_advance);
EXPORT_SYMBOL(cl_io_lock_clock);
EXPORT_SYMBOL(cl_io_iter_init_clock);
EXPORT_SYMBOL(cl_io_start_clock);
EXPORT_SYMBOL(cl_io_end_clock);
EXPORT_SYMBOL(cl_io_unlock_clock);
EXPORT_SYMBOL(cl_io_iter_fini_clock);
EXPORT_SYMBOL(cl_io_rw_advance_clock);
 ...
 */
int cl_io_loop_internal(const struct lu_env *env, struct cl_io *io)
{
	int result = 0;
	int rc = 0;
	...
	do {
		...
		result = cl_io_iter_init(env, io);
		ktget(&localclock[1]);
		ktput(localclock, cl_io_iter_init);
		if (result == 0) {
			nob    = io->ci_nob;
			ktget(&localclock[0]);
			result = cl_io_lock(env, io);
			ktget(&localclock[1]);
			ktput(localclock, cl_io_lock);
			if (result == 0) {
				/*
				 * Notify layers that locks has been taken,
				 * and do actual i/o.
				...
				cl_io_end(env, io);
				ktget(&localclock[1]);
				ktput(localclock, cl_io_end);

				ktget(&localclock[0]);
				cl_io_unlock(env, io);
				ktget(&localclock[1]);
				ktput(localclock, cl_io_unlock);

				ktget(&localclock[0]);
				cl_io_rw_advance(env, io, io->ci_nob - nob);
				ktget(&localclock[1]);
				ktput(localclock, cl_io_rw_advance);
			}
		}
		ktget(&localclock[0]);
		cl_io_iter_fini(env, io);
		ktget(&localclock[1]);
		ktput(localclock, cl_io_iter_fini);
		if (result)
			rc = result;
	} while ((result == 0 || result == -EIOCBQUEUED) &&
		 io->ci_continue);
...
KTDEF(cl_io_loop_internal);
EXPORT_SYMBOL(cl_io_loop_internal_clock);

int cl_io_loop(const struct lu_env *env, struct cl_io *io){
	int ret;
	ret = cl_io_loop_internal(env, io);
	return ret;
}
EXPORT_SYMBOL(cl_io_loop);

int cl_io_loop_profile(const struct lu_env *env, struct cl_io *io){
	// ktime_t localclock[2];
	int ret;
	// ktget(&localclock[0]);
	ret = cl_io_loop_internal(env, io);
	// ktget(&localclock[1]);
	// ktput(localclock, cl_io_loop_internal);
	return ret;
}
EXPORT_SYMBOL(cl_io_loop_profile);
...
KTDEF(cl_sync_io_wait_event_idle);
EXPORT_SYMBOL(cl_sync_io_wait_event_idle_clock);
		    ...
		    long timeout)
{
	...
	wait_event_idle(anchor->csi_waitq,
			atomic_read(&anchor->csi_sync_nr) == 0);
	// while (!atomic_read(&anchor->csi_sync_nr) == 0)
	// {
	// 	cpu_relax();
	// }
	ktget(&localclock[1]);
	ktput(localclock, cl_sync_io_wait_event_idle);
	if (!rc)
...
KTDEF(cl_sync_io_wait);
EXPORT_SYMBOL(cl_sync_io_wait_clock);
			    ...
			    long timeout, int ioret)
{
	...
	rc = cl_sync_io_wait(env, anchor, timeout);
	ktget(&localclock[1]);
	ktput(localclock, cl_sync_io_wait);
	/**
