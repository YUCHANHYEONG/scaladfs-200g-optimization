...
static void ptlrpc_prep_bulk_page_pin_mulitple_md(struct ptlrpc_bulk_desc *desc,
				      struct page *page, int pageoffset,
				      int len)
{
	__ptlrpc_prep_bulk_page_md(desc, page, pageoffset, len, 1);
}
...
const struct ptlrpc_bulk_frag_ops ptlrpc_bulk_kiov_multiple_md_ops = {
	.add_kiov_frag	= ptlrpc_prep_bulk_page_pin_mulitple_md,
	.release_frags	= ptlrpc_release_bulk_page_pin,
};
EXPORT_SYMBOL(ptlrpc_bulk_kiov_multiple_md_ops);
...
static int application_unregister_reply(struct ptlrpc_request *request, int async);
...
struct ptlrpc_bulk_desc *ptlrpc_new_bulk_md(unsigned int nfrags,
					 unsigned int max_brw,
					 enum ptlrpc_bulk_op_type type,
					 unsigned int portal,
					 const struct ptlrpc_bulk_frag_ops *ops)
...
struct ptlrpc_bulk_desc *ptlrpc_prep_bulk_imp_multiple_md(struct ptlrpc_request *req,
					      unsigned int nfrags,
					      unsigned int max_brw,
					      unsigned int type,
					      unsigned int portal,
						...
						*ops)
{
	...
	desc = ptlrpc_new_bulk_md(nfrags, max_brw, type, portal, ops);
	if (!desc)
		RETURN(NULL);
...
EXPORT_SYMBOL(ptlrpc_prep_bulk_imp_multiple_md);
...
void __ptlrpc_prep_bulk_page_md(struct ptlrpc_bulk_desc *desc,
			     struct page *page, int pageoffset, int len,
			     int pin)
{
	struct bio_vec *kiov;
	...
	// if (((desc->bd_iov_count % LNET_MAX_IOV) == 0) ||
	// if (((desc->bd_iov_count % 128) == 0) ||
	//      ((desc->bd_nob_last + len) > LNET_MTU)) {
	// 	desc->bd_mds_off[desc->bd_md_count] = desc->bd_iov_count;
	// 	desc->bd_md_count++;
	// 	pr_info("bd_md_count: %d, iov_count: %d\n", desc->bd_md_count, desc->bd_iov_count);
	// 	// pr_info("LNET_MTU: %d\n", LNET_MTU);
	// 	// pr_info("desc->bd_nob_last + len: %d\n", desc->bd_nob_last + len);
	// 	desc->bd_nob_last = 0;
	// 	// LASSERT(desc->bd_md_count <= PTLRPC_BULK_OPS_COUNT);
...
EXPORT_SYMBOL(__ptlrpc_prep_bulk_page_md);

// void __ptlrpc_prep_bulk_page_modified(struct ptlrpc_bulk_desc *desc,
//                  struct page *page, int pageoffset, int len,
//                  int pin)
// {
//     struct bio_vec *kiov;
...
KTDEF(obd_get_mod_rpc_slot);
EXPORT_SYMBOL(obd_get_mod_rpc_slot_clock);
...
void ptlrpc_get_mod_rpc_slot_internal(struct ptlrpc_request *req)
{
	struct client_obd *cli = &req->rq_import->imp_obd->u.cli;
	__u32 opc;
	__u16 tag;
	...
	ktget(&localclock[0]);
	//tag = obd_get_mod_rpc_slot(cli, opc);
	tag = obd_get_mod_rpc_slot_internal(cli, opc);
	ktget(&localclock[1]);
	ktput(localclock, obd_get_mod_rpc_slot);
	lustre_msg_set_tag(req->rq_reqmsg, tag);
	ptlrpc_reassign_next_xid(req);
}
EXPORT_SYMBOL(ptlrpc_get_mod_rpc_slot_internal);
...
EXPORT_SYMBOL(application_prep_set);
...
void application_set_add_new_req(struct ptlrpc_request *req,
			    struct ptlrpc_request_set *set)
{
	int count;

	LASSERT(req->rq_set == NULL);
	// pr_info("application_set_add_new_req\n");
...
KTDEF(application_check_set);
EXPORT_SYMBOL(application_check_set_clock);
...
int application_check_req(struct lu_env *env, struct ptlrpc_request_set *set)
{
	...
	if (rc2 != 0) {
		pr_info("Error: Failure to refill session: %d\n", rc2);
		/*
	...
	while (atomic_read(&set->set_remaining)){
	// if (atomic_read(&set->set_remaining)){
		// pr_info("set->set_remaining: %d\n", atomic_read(&set->set_remaining));
		ktget(&localclock[0]);
		rc |= application_check_set(env, set);
		ktget(&localclock[1]);
		ktput(localclock, application_check_set);
	}
...
EXPORT_SYMBOL(application_check_req);
 ...
 * Returns 0 on success or error code.
 */
static int ptlrpc_send_new_req_internal(struct ptlrpc_request *req)
{
	struct obd_import *imp = req->rq_import;
	__u64 min_xid = 0;
	int rc;
	...
	// pr_info("Sending RPC req@%p pname:cluuid:pid:xid:nid:opc:job %s:%s:%d:%llu:%s:%d:%s\n",
	//        req, current->comm,
	//        imp->imp_obd->obd_uuid.uuid,
	//        lustre_msg_get_status(req->rq_reqmsg), req->rq_xid,
	//        obd_import_nid2str(imp), lustre_msg_get_opc(req->rq_reqmsg),
	...
	rc = ptl_send_rpc_combined(req, 0);
	if (rc == -ENOMEM) {
		spin_lock(&imp->imp_lock);
...
KTDEF(ptlrpc_send_new_req);
EXPORT_SYMBOL(ptlrpc_send_new_req_clock);
...
static int ptlrpc_send_new_req(struct ptlrpc_request *req)
{
	...
	ktime_t localclock[2];
	ktget(&localclock[0]);
	ret = ptlrpc_send_new_req_internal(req);
	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_send_new_req);
	return ret;
}
...
KTDEF(rq_lock);
EXPORT_SYMBOL(rq_lock_clock);

KTDEF(ptlrpc_check_set_rq_lock);
EXPORT_SYMBOL(ptlrpc_check_set_rq_lock_clock);

KTDEF(ptlrpc_req_interpret);
EXPORT_SYMBOL(ptlrpc_req_interpret_clock);
 ...
 */
int ptlrpc_check_set_internal(const struct lu_env *env, struct ptlrpc_request_set *set)
{
	...
	list_for_each_entry_safe(req, next, &set->set_requests,
		...
		if (req->rq_phase == RQ_PHASE_RPC) {
			...
			spin_lock(&req->rq_lock);
			ktget(&localclock[1]);
			ktput(localclock, ptlrpc_check_set_rq_lock);
...
interpret:
		...
		if (!unregistered && !ptlrpc_unregister_reply(req, async)) {
			...
			ptlrpc_unregister_bulk(req, 1);
			// ktget(&localclock[1]);
			// ktput(localclock, ptlrpc_req_interpret);
			continue;
		}
...
KTDEF(kiet_spin_lock);
EXPORT_SYMBOL(kiet_spin_lock_clock);

// static void kiet_spin_lock(spinlock_t *lock)
// {
// 	ktime_t localclock[2];
// 	ktget(&localclock[0]);
// 	spin_lock(lock);
// 	ktget(&localclock[1]);
// 	ktput(localclock, kiet_spin_lock);
// }

KTDEF(application_check_set_INTERPRET);
EXPORT_SYMBOL(application_check_set_INTERPRET_clock);

KTDEF(application_check_set_RPC);
EXPORT_SYMBOL(application_check_set_RPC_clock);

KTDEF(application_check_set_bot);
EXPORT_SYMBOL(application_check_set_bot_clock);

KTDEF(application_check_set_bot_1);
EXPORT_SYMBOL(application_check_set_bot_1_clock);

KTDEF(application_cond_resched);
EXPORT_SYMBOL(application_cond_resched_clock);

KTDEF(application_check_set_BULK);
EXPORT_SYMBOL(application_check_set_BULK_clock);

KTDEF(application_check_set_middle);
EXPORT_SYMBOL(application_check_set_middle_clock);

KTDEF(application_check_set_middle_1);
EXPORT_SYMBOL(application_check_set_middle_1_clock);

KTDEF(application_check_set_atomic);
EXPORT_SYMBOL(application_check_set_atomic_clock);

KTDEF(application_check_set_for);
EXPORT_SYMBOL(application_check_set_for_clock);

int application_check_set(const struct lu_env *env, struct ptlrpc_request_set *set)
{
	bool ret;
	struct ptlrpc_request *req, *next;
	LIST_HEAD(comp_reqs);
	...
	if (atomic_read(&set->set_remaining) == 0) {
		ktget(&localclock[1]);
		ktput(localclock, application_check_set_atomic);
		RETURN(1);
	}
	ktget(&localclock[1]);
	ktput(localclock, application_check_set_atomic);
	...
	list_for_each_entry_safe(req, next, &set->set_requests,
		...
		// atomic_cmpxchg(&req->application, 0, 1); // for application flush
		...
		if (unlikely(req->rq_allow_intr && req->rq_intr)) {
			...
			ktget(&localclock[1]);
			ktput(localclock, application_cond_resched);
			GOTO(interpret, req->rq_status);
		}
		ktget(&localclock[1]);
		ktput(localclock, application_cond_resched);
		...
		if (req->rq_phase == RQ_PHASE_NEW && ptlrpc_send_new_req(req)){
			force_timer_recalc = 1;
			ktget(&localclock[1]);
			ktput(localclock, application_check_set_bot_1);
		}
		ktget(&localclock[1]);
		ktput(localclock, application_check_set_bot_1);
...
//		ktput(localclock, application_check_set_bot_1);
		...
		if (req->rq_phase == RQ_PHASE_UNREG_RPC ||
			...
			if (req->rq_phase == RQ_PHASE_UNREG_RPC &&
			    ptlrpc_cli_wait_unlink(req)){
				ktget(&localclock[1]);
				ktput(localclock, application_check_set_bot);
				continue;
			}
			if (req->rq_phase == RQ_PHASE_UNREG_BULK &&
			    ptlrpc_client_bulk_active(req)){
		...
		if (req->rq_phase == RQ_PHASE_INTERPRET) {
			ktget(&localclock[1]);
			ktput(localclock, application_check_set_RPC);
			GOTO(interpret, req->rq_status);
		}
		 ...
		 */
		if (req->rq_net_err && !req->rq_timedout) {
			pr_info("req->rq_net_err && !req->rq_timedout\n");
			ptlrpc_expire_one_request(req, 1);
		...
		ktget(&localclock[1]);
		ktput(localclock, application_check_set_RPC); // why comment?
		...
		if (req->rq_err) {
			...
			if (!ptlrpc_unregister_reply(req, 1)) {
				ptlrpc_unregister_bulk(req, 1);
				ktget(&localclock[1]);
				ktput(localclock, application_check_set_middle);
				continue;
			}
		...
		if (req->rq_phase == RQ_PHASE_RPC) {
			 ...
			 */
			// unregistered = ptlrpc_unregister_reply(req, 1);
			unregistered = application_unregister_reply(req, 1);
			if (!unregistered) {
				ktget(&localclock[1]);
				ktput(localclock, application_check_set_middle_1);
				continue;
		...
		if (ptlrpc_client_bulk_active(req)){
			ktget(&localclock[1]);
			ktput(localclock, application_check_set_BULK);
			continue;
		}
...
interpret:
		...
		if (!unregistered && !ptlrpc_unregister_reply(req, async)) {
			...
			ptlrpc_unregister_bulk(req, 1);
			ktget(&localclock[1]);
			ktput(localclock, application_check_set_INTERPRET);
			continue;
		}
		...
		// pr_info("Start to interpret \n");
		ptlrpc_req_interpret(env, req, req->rq_status);
		// pr_info("Done interpret \n");
	...
	ktget(&forclock[1]);
	ktput(forclock, application_check_set_for);
	...
	ktget(&localclock[1]);
	ktput(localclock, application_check_set_atomic);
	RETURN(ret || force_timer_recalc);
}

KTDEF(ptlrpc_check_set);
EXPORT_SYMBOL(ptlrpc_check_set_clock);

int ptlrpc_check_set(const struct lu_env *env, struct ptlrpc_request_set *set){
	ktime_t localclock[2];
	int ret;
	ktget(&localclock[0]);
	ret = ptlrpc_check_set_internal(env, set);
	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_check_set);
	return ret;
}
...
KTDEF(ptlrpc_wait_event_abortable);
EXPORT_SYMBOL(ptlrpc_wait_event_abortable_clock);
KTDEF(ptlrpc_wait_event_idle);
EXPORT_SYMBOL(ptlrpc_wait_event_idle_clock);
...
int ptlrpc_set_wait(const struct lu_env *env, struct ptlrpc_request_set *set)
{
	...
	do {
		...
		if ((timeout == 0 && !signal_pending(current)) ||
			...
			ktget(&localclock[1]);
			ktput(localclock, ptlrpc_wait_event_abortable);
			if (rc == 0) {
				rc = -ETIMEDOUT;
			...
			rc = wait_event_idle_timeout(
				...
				ptlrpc_check_set(NULL, set),
				cfs_time_seconds(timeout ? timeout : 1));
			// 	cfs_time_seconds(timeout ? 1 : 1));
			ktget(&localclock[1]);
			ktput(localclock, ptlrpc_wait_event_idle);
			if (rc == 0) {
				ptlrpc_expired_set(set);
				rc = -ETIMEDOUT;
			} else {
...
KTDEF(ptlrpc_wait_event_abortable_mdt);
EXPORT_SYMBOL(ptlrpc_wait_event_abortable_mdt_clock);
KTDEF(ptlrpc_wait_event_idle_mdt);
EXPORT_SYMBOL(ptlrpc_wait_event_idle_mdt_clock);
 ...
 * Returns 0 on success or error code otherwise.
 */
int ptlrpc_set_wait_mdt(const struct lu_env *env, struct ptlrpc_request_set *set)
{
	struct ptlrpc_request *req;
	time64_t timeout;
	int rc;
	...
	do {
		...
		if ((timeout == 0 && !signal_pending(current)) ||
			...
			rc = l_wait_event_abortable_timeout(
				...
				cfs_time_seconds(timeout ? timeout : 1));
			ktget(&localclock[1]);
			ktput(localclock, ptlrpc_wait_event_abortable_mdt);
			if (rc == 0) {
				rc = -ETIMEDOUT;
				ptlrpc_expired_set(set);
			} else if (rc < 0) {
			...
			// 	ptlrpc_check_set(NULL, set),
			// 	cfs_time_seconds(timeout ? timeout : 1));
			while (!ptlrpc_check_set(NULL, set)){
				cpu_relax();
			}
			ktget(&localclock[1]);
			ktput(localclock, ptlrpc_wait_event_idle_mdt);
			if (rc == 0) {
				ptlrpc_expired_set(set);
				pr_info("RPC is expired \n");
				rc = -ETIMEDOUT;
			} else {
				rc = 0;
			}
...
EXPORT_SYMBOL(ptlrpc_set_wait_mdt);
...
static int application_unregister_reply(struct ptlrpc_request *request, int async)
{
	...
	// if (atomic_read(&request->application) == 2){
	// 	application_handling_md(request->rq_reply_md_h);
	// }
	LNetMDUnlink(request->rq_reply_md_h);
...
KTDEF(ptlrpc_set_wait);
EXPORT_SYMBOL(ptlrpc_set_wait_clock);
...
int ptlrpc_queue_wait(struct ptlrpc_request *req)
{
	...
	ktget(&localclock[1]);
	ktput(localclock, ptlrpc_set_wait);
...
int ptlrpc_queue_wait_mdt(struct ptlrpc_request *req)
{
	struct ptlrpc_request_set *set;
	int rc;
	...
	ptlrpc_request_addref(req);
	ptlrpc_set_add_req(set, req);
	rc = ptlrpc_set_wait_mdt(NULL, set);
	ptlrpc_set_destroy(set);

	RETURN(rc);
}
EXPORT_SYMBOL(ptlrpc_queue_wait_mdt);
