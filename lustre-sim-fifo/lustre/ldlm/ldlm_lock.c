...
KTDEF(lock_res_and_lock);
EXPORT_SYMBOL(lock_res_and_lock_clock);

KTDEF(ldlm_lock_add_to_lru);
EXPORT_SYMBOL(ldlm_lock_add_to_lru_clock);
...
void lustre_ldlm_lock_decref_internal(struct ldlm_lock *lock, enum ldlm_mode mode)
{
	...
	lock_res_and_lock(lock);
	ktget(&localclock[1]);
	ktput(localclock, lock_res_and_lock);
	...
	if (!lock->l_readers && !lock->l_writers && ldlm_is_cbpending(lock)) {
		...
		ktget(&localclock[1]);
		ktput(localclock, ldlm_lock_add_to_lru);
		unlock_res_and_lock(lock);
...
repeat:
	...
	lock_res(res);
	ktget(&localclock[1]);
	ktput(localclock, lock_res);
	if (res->lr_type == LDLM_EXTENT){
		ktget(&localclock[0]);
		lock = search_itree(res, &data);
		// if (lock)
		// 	pr_info("Ext start and end %llu %llu\n", data.lmd_policy->l_extent.start, data.lmd_policy->l_extent.start);
		ktget(&localclock[1]);
