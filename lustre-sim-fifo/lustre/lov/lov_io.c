...
KTDEF(lov_io_start);
EXPORT_SYMBOL(lov_io_start_clock);

int fsync_cl_io_start(const struct lu_env *env, struct cl_io *io);
static int fsync_lov_io_start(const struct lu_env *env, const struct cl_io_slice *ios)
{
	int ret;
	...
	ktget(&localclock[0]);
	ret = lov_io_call(env, cl2lov_io(env, ios), fsync_cl_io_start);
	//ret = lov_io_call(env, cl2lov_io(env, ios), cl_io_start);
	ktget(&localclock[1]);
	ktput(localclock, lov_io_start);
	RETURN(ret);
